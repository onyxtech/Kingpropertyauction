...
import { CheckCircle, Save, Loader2, Building2, AlertCircle } from "lucide-react";
...
import { showSuccess as toastSuccess, showError as toastError } from "@/lib/toast";

export default function ProfileTab() {
  const theme = useTheme();
  const { user, login, token } = useAuthStore();
  const { role, getCombinedRoleLabel, canBid, canListProperties } = useCustomerRole();
  ...
  const defaultNotifSettings = {
    bidPlaced: true, outbid: true, auctionWon: true, auctionLost: true,
  ...
  useEffect(() => {
    ...
    if (freshProfile.agentDetails) {
      setFormData(f => ({
        ...
        licenseNumber: freshProfile.agentDetails?.licenseNumber || f.licenseNumber,
        companyAddress: freshProfile.agentDetails?.companyAddress || f.companyAddress,
      }));
    }
  }, [freshProfile]);
  ...
  const handleSave = async () => {
    setSaveError("");
    try {
      const agentDetails = (role === "agent" || role === "seller") ? {
        companyName: formData.companyName,
        licenseNumber: formData.licenseNumber,
        companyAddress: formData.companyAddress,
      } : undefined;
      const payload = { name: formData.name, phone: formData.phone, ...(agentDetails ? { agentDetails } : {}) };
      await updateProfile.mutateAsync(payload);
      login(token!, { ...user!, name: formData.name, phone: formData.phone, ...(agentDetails ? { agentDetails } : {}) });
      showSuccess();
  ...
  const handleBankSave = async () => {
    setBankError("");
    if (!bankForm.accountHolderName.trim()) { setBankError("Account holder name is required"); return; }
    if (!bankForm.bankName.trim()) { setBankError("Bank name is required"); return; }
    if (!bankForm.accountNumber.trim()) { setBankError("Account number is required"); return; }
    if (bankForm.accountNumber.replace(/\s/g, "").length < 6) { setBankError("Please enter a valid account number"); return; }
    if (!bankForm.sortCode.trim()) { setBankError("Sort code is required"); return; }
    try {
  ...
  const handleNotifSave = async () => {
    ...
    try {
      ...
      setNotifError(err?.message || "Failed to save settings. Please try again.");
  ...
  const handlePasswordChange = async () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        toastError("Password change failed", result.message || "Failed to change password");
      }
    } catch { setPwError("Failed to change password"); toastError("Password change failed", "Please try again."); }
  };
  ...
  const SaveButton = ({ onClick, label = "Save Changes" }: { onClick: () => void; label?: string }) => (
    ...
    >
      {updateProfile.isPending ? <><Loader2 className="size-5 animate-spin" /> Saving...</> : <><Save className="size-5" /> {label}</>}
    </button>
  );
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-slate-600 font-medium">Manage your account information</p>
      </div>
      ...
      {activeSection === "profile" && (
        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-6 space-y-6">
          ...
          <div className="flex items-start gap-6">
            <div className={`size-20 rounded-2xl bg-gradient-to-br ${theme.primary} flex items-center justify-center shadow-lg flex-shrink-0`}>
              <span className="text-white font-black text-3xl">{user?.name?.charAt(0)?.toUpperCase() || "U"}</span>
            </div>
            <div>
              <h3 className="text-xl font-black text-slate-900 mb-1">{user?.name}</h3>
              <div className="flex items-center gap-2 mb-2">
                <CheckCircle className="size-4 text-green-600" />
                <span className="text-slate-600 font-medium text-sm">Verified Member</span>
              </div>
              <span className={`px-3 py-1 rounded-full text-sm font-bold ${roleBadgeColor()}`}>{getCombinedRoleLabel()}</span>
              <p className="text-xs text-slate-400 mt-2">Member since {user?.createdAt ? new Date(user.createdAt).toLocaleDateString() : "Recently"}</p>
            </div>
          </div>
          ...
          <div className="grid md:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <input type="text" value={getCombinedRoleLabel()} disabled
                className="w-full px-4 py-3 bg-slate-100 border-2 border-slate-200 rounded-xl font-medium text-slate-500" />
            </div>
          </div>
          ...
          <div className="flex gap-4">
            ...
            <button onClick={() => setFormData({ name: user?.name || "", email: user?.email || "", phone: user?.phone || "", companyName: user?.agentDetails?.companyName || "", licenseNumber: user?.agentDetails?.licenseNumber || "", companyAddress: user?.agentDetails?.companyAddress || "" })}
        ...
        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-6 space-y-4">
          <div className="grid md:grid-cols-2 gap-4">
            <div>
              ...
              <input type="text" value={formData.companyName} onChange={e => setFormData(f => ({ ...f, companyName: e.target.value }))}
            ...
            <div className="md:col-span-2">
              <AddressAutocomplete
                ...
                onChange={(value) => setFormData(f => ({ ...f, companyAddress: value }))}
                onAddressSelect={(parsed) => {
                  const parts = [parsed.streetAddress, parsed.city, parsed.state, parsed.postalCode, parsed.country].filter(Boolean);
                  setFormData(f => ({ ...f, companyAddress: parts.join(", ") }));
                }}
        ...
        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-6 space-y-4">
          <div className="flex items-center gap-3 p-4 bg-amber-50 border border-amber-200 rounded-xl">
            ...
            <p className="text-sm font-bold text-amber-700">
              Your bank details are stored securely and only used for property sale payments.
            </p>
          </div>
          <div className="grid md:grid-cols-2 gap-4">
            <div>
              <label className="block text-sm font-bold text-slate-700 mb-1">Account Holder Name</label>
              <input type="text" value={bankForm.accountHolderName} onChange={e => setBankForm(f => ({ ...f, accountHolderName: e.target.value }))}
                placeholder="Full name on bank account"
                className="w-full px-4 py-3 border-2 border-slate-200 rounded-xl text-sm font-medium focus:outline-none focus:ring-2 focus:ring-blue-500" />
            </div>
            <div>
              <label className="block text-sm font-bold text-slate-700 mb-1">Bank Name</label>
              <input type="text" value={bankForm.bankName} onChange={e => setBankForm(f => ({ ...f, bankName: e.target.value }))}
            ...
            <div>
              ...
              <input type="text" value={bankForm.accountNumber} onChange={e => setBankForm(f => ({ ...f, accountNumber: e.target.value }))}
                placeholder="8 digit account number" maxLength={8}
                className="w-full px-4 py-3 border-2 border-slate-200 rounded-xl text-sm font-medium focus:outline-none focus:ring-2 focus:ring-blue-500" />
            </div>
            <div>
              <label className="block text-sm font-bold text-slate-700 mb-1">Sort Code</label>
              <input type="text" value={bankForm.sortCode} onChange={e => setBankForm(f => ({ ...f, sortCode: e.target.value }))}
                placeholder="XX-XX-XX" maxLength={8}
                className="w-full px-4 py-3 border-2 border-slate-200 rounded-xl text-sm font-medium focus:outline-none focus:ring-2 focus:ring-blue-500" />
            </div>
            <div className="md:col-span-2">
              <label className="block text-sm font-bold text-slate-700 mb-1">IBAN (Optional)</label>
              <input type="text" value={bankForm.iban} onChange={e => setBankForm(f => ({ ...f, iban: e.target.value }))}
                placeholder="International bank account number"
                className="w-full px-4 py-3 border-2 border-slate-200 rounded-xl text-sm font-medium focus:outline-none focus:ring-2 focus:ring-blue-500" />
            </div>
            <div className="md:col-span-2">
              <label className="block text-sm font-bold text-slate-700 mb-1">Bank Address (Optional)</label>
              <input type="text" value={bankForm.bankAddress} onChange={e => setBankForm(f => ({ ...f, bankAddress: e.target.value }))}
      ...
      {activeSection === "notifications" && (
        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-6 space-y-4">
          <p className="text-slate-600 font-medium">Control which notifications you receive in your bell.</p>
          <div className="space-y-3">
            {[
              ...(canBid ? [
                { key: "bidPlaced", label: "Bid Placed Confirmation", desc: "When you successfully place a bid" },
                { key: "outbid", label: "Outbid Alert", desc: "When someone outbids you" },
                { key: "auctionWon", label: "Auction Won", desc: "When you win an auction" },
                { key: "auctionLost", label: "Auction Lost", desc: "When auction ends and you didn't win" },
                { key: "auctionEnded", label: "Auction Ended", desc: "When an auction you bid in ends" },
                { key: "offerReceived", label: "Property Offer Received", desc: "When admin sends you a post-auction property offer" },
              ] : []),
              ...(canListProperties ? [
                { key: "propertyApproved", label: "Property Approved", desc: "When admin approves your property" },
                { key: "propertyRejected", label: "Property Rejected", desc: "When admin rejects your property" },
                { key: "propertySold", label: "Property Sold", desc: "When your property sells at auction" },
                { key: "newBidOnProperty", label: "New Bid on Property", desc: "When someone bids on your property" },
                { key: "auctionStarted", label: "Auction Started", desc: "When auction with your property goes live" },
                { key: "auctionEnded", label: "Auction Ended", desc: "When auction containing your property ends" },
                { key: "newEnquiry", label: "New Property Enquiry", desc: "When someone enquires about your property" },
              ] : []),
              { key: "messageReceived", label: "New Message", desc: "When you receive a new message" },
            ].map(({ key, label, desc }) => (
              <div key={key} className="flex items-center justify-between p-4 bg-slate-50 rounded-xl border border-slate-200">
                ...
                <button
                  ...
                  className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors flex-shrink-0 ml-4 ${
                    notifSettings[key as keyof typeof notifSettings] ? "bg-blue-500" : "bg-slate-300"
                  }`}
                >
                  <span className={`inline-block size-4 bg-white rounded-full shadow transition-transform ${
        ...
        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-6 space-y-4">
          ...
          {pwSaved && (
            <div className="p-3 bg-green-50 border border-green-200 rounded-xl flex items-center gap-2">
              <CheckCircle className="size-4 text-green-600" />
              <p className="text-sm font-bold text-green-700">Password changed successfully!</p>
            </div>
          )}
          <div className="grid md:grid-cols-3 gap-4">
            <div>
              <label className="block text-sm font-bold text-slate-700 mb-1">Current Password</label>
              <input type="password" value={pwForm.currentPassword} onChange={e => { setPwError(""); setPwForm(f => ({ ...f, currentPassword: e.target.value })); }}
                className="w-full px-4 py-3 bg-white border-2 border-slate-200 rounded-xl font-medium focus:outline-none focus:ring-2 focus:ring-blue-500" />
            </div>
            <div>
              <label className="block text-sm font-bold text-slate-700 mb-1">New Password</label>
              <input type="password" value={pwForm.newPassword} onChange={e => { setPwError(""); setPwForm(f => ({ ...f, newPassword: e.target.value })); }}
                className="w-full px-4 py-3 bg-white border-2 border-slate-200 rounded-xl font-medium focus:outline-none focus:ring-2 focus:ring-blue-500" />
            </div>
            <div>
              <label className="block text-sm font-bold text-slate-700 mb-1">Confirm Password</label>
              <input type="password" value={pwForm.confirmPassword} onChange={e => { setPwError(""); setPwForm(f => ({ ...f, confirmPassword: e.target.value })); }}
                className="w-full px-4 py-3 bg-white border-2 border-slate-200 rounded-xl font-medium focus:outline-none focus:ring-2 focus:ring-blue-500" />
            </div>
          </div>
