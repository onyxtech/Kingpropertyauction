...
const INITIAL_MESSAGE: Message = {
  ...
  role: "assistant",
  content:
    "👋 Hi! I'm the King Property Auction AI Assistant.\n\nI can help with:\n• Finding properties & auctions\n• How auctions work\n• Listing your property\n• Registration & bidding\n• General enquiries\n\nPlease tell me your name, email and mobile number to get started.",
  timestamp: new Date(),
};
...
export default function AIChatWidget() {
  ...
  useEffect(() => {
    ...
    if (isAuthenticated && user) {
      ...
      visitorPhoneRef.current = (user as any).phone || "";
    } else {
  ...
  const handleContinuePrevious = useCallback(async () => {
    ...
    visitorPhoneRef.current = previousSession.visitorPhone || "";
    setPhoneInput(previousSession.visitorPhone || "");
    setHasProvidedInfo(true);
    setShowPreviousSession(false);

    try {
      const res = await fetch(
        `/api/chat/history/${previousSession.conversationId}`,
      );
      ...
      if (data.success && data.data.messages?.length > 0) {
        ...
        setMessages(
          previousSession.messages.map((m) => ({
            ...m,
  ...
  const handleSubmitInfo = useCallback(() => {
    ...
    fetch("/api/leads", {
      ...
      body: JSON.stringify({
        ...
        message: `Chat initiated by ${name} (${email})${phone ? ` - Phone: ${phone}` : ""}`,
        leadType: "chat",
      }),
    }).catch(() => {}); // Fire and forget - don't block the chat
  ...
  const streamAIResponse = useCallback(
    async (userText: string, department?: string) => {
      if (isLoading) return;
      setIsLoading(true);
      ...
      try {
        ...
        const response = await fetch("/api/chat/ai", {
          ...
          body: JSON.stringify({
            ...
            visitorPhone: visitorPhoneRef.current || undefined,
          ...
          prev.map((m) =>
            m.id === assistantMsgId
              ? {
                  ...m,
                  content:
                    "Sorry, I'm having trouble right now. Please call us on 0800 123 4567 or use the contact form.",
                  isStreaming: false,
                }
              : m,
          ),
  ...
  return (
    <div className="fixed bottom-6 right-6 z-50">
      ...
      {isOpen && (
        <div
          className={`bg-white rounded-3xl shadow-2xl border border-slate-200 flex flex-col overflow-hidden transition-all duration-200 ${
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <div className="flex items-center gap-1.5">
                  <span
                    className={`size-1.5 rounded-full ${agentMode ? "bg-yellow-300" : "bg-green-400"} animate-pulse`}
                  />
                  <span className="text-[11px] text-white/80 font-medium">
                    {agentMode
                      ? "Agent connected · King Property Auction"
                      : "Online · King Property Auction"}
                  </span>
                </div>
              </div>
            </div>
              ...
              {agentMode && (
                <div className="bg-emerald-50 border-b border-emerald-200 px-4 py-2 flex items-center gap-2 flex-shrink-0">
                  ...
                  <span className="text-xs font-semibold text-emerald-700">
                    You're now connected to our support team. An agent will
                    respond shortly.
                  </span>
                </div>
              )}
              ...
              <div className="flex-1 overflow-y-auto px-4 py-4 space-y-4 bg-slate-50">
                {messages.map((msg) => (
                  ...
                  >
                    {msg.role === "assistant" && (
                      <div
                        className={`size-7 rounded-full flex items-center justify-center flex-shrink-0 mb-1 ${
                          msg.isAgent
                    ...
                    <div
                      className={`max-w-[78%] flex flex-col ${msg.role === "user" ? "items-end" : "items-start"}`}
                    >
                      <div
