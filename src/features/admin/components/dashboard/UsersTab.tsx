...
const getUserRoleLabel = (user: any) => {
  ...
  if (user.role === "agent") {
    if (canBid && canList)
      return { label: "Agent & Buyer", color: "bg-purple-100 text-purple-700" };
    return { label: "Agent", color: "bg-orange-100 text-orange-700" };
  }
  if (user.role === "seller") {
    if (canBid && canList)
      return { label: "Buyer & Owner", color: "bg-teal-100 text-teal-700" };
    return { label: "Owner", color: "bg-blue-100 text-blue-700" };
  }
    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-slate-600 font-medium">
            Manage all registered users by role
          </p>
        </div>
        <div className="flex items-center gap-3">
          ...
          >
            <UserCheck className="size-5" /> Add Agent
          </button>
          <button
            onClick={onAddUser}
      ...
      <div className="grid grid-cols-1 md:grid-cols-5 gap-4 mb-4">
        {[
          ...
          {
            label: "Active Users",
            value: allUsers.filter((u: any) => u.isActive).length,
            color: "from-green-500 to-emerald-600",
          },
          {
            label: "Buyers",
            value: allUsers.filter(
              (u: any) => u.role === "user" || u.role === "buyer",
            ).length,
            color: "from-purple-500 to-pink-600",
          },
          {
            label: "Agents",
            value: allUsers.filter((u: any) => u.role === "agent").length,
                    ...
                    <td className="px-6 py-4">
                      <div className="flex flex-col gap-1">
                        {(() => {
                          ...
                          return (
                            <span
                              className={`px-3 py-1.5 rounded-lg text-xs font-bold inline-block w-fit ${roleInfo.color}`}
                            >
                              {roleInfo.label}
                            </span>
                          );
          ...
          <div className="flex items-center justify-between px-6 py-4 border-t border-slate-200 bg-slate-50">
            ...
            <div className="flex items-center gap-2">
              <button
                ...
                disabled={page === 1}
                className="p-2 bg-white border-2 border-slate-200 rounded-xl disabled:opacity-50"
              >
