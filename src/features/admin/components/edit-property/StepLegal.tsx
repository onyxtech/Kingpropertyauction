...
import {
  ...
  Lock, User, Phone, Mail, Building2, Trash2,
} from "lucide-react";
...
const getDocLabel = (doc: any) => {
  ...
  return DOC_TYPES.find(d => d.value === dt)?.label || (dt ? dt.replace(/_/g, " ") : "📄 Document");
};

export default function StepLegal({ form, updateField, propertyId }: any) {
  ...
  const removeNewDoc = (i: number) => {
    updateField("newPrivateDocs", newDocs.filter((_: any, idx: number) => idx !== i));
  };
  ...
  const deleteExistingDoc = async (docIndex: number) => {
    ...
    try {
      ...
      if (data.success) {
        updateField("existingPrivateDocs", existingDocs.filter((_: any, i: number) => i !== docIndex));
        showSuccess("Document deleted", "The document has been removed.");
      } else {
        showError("Delete failed", data.message || "Could not delete document.");
      }
    } catch {
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-2xl border-2 border-slate-200 p-5">
        ...
        <div className="grid md:grid-cols-2 gap-4">
          <div>
            <label className="block text-sm font-bold mb-1">Ownership Type</label>
            <select
          ...
          <div>
            <label className="block text-sm font-bold mb-1">Title Deed Number</label>
            <input
              type="text"
      ...
      <div className="bg-white rounded-2xl border-2 border-slate-200 p-5">
        ...
        <div className="grid md:grid-cols-2 gap-4">
          <div>
            <label className="block text-sm font-bold mb-1">Solicitor Name</label>
          ...
          <div>
            <label className="block text-sm font-bold mb-1">Firm / Company Name</label>
            ...
            <AddressAutocomplete
              ...
              onAddressSelect={(addr: ParsedAddress) => {
                ...
                updateField("solicitorDetails", {
                  ...
                  address: parts.join(", "),
                  postcode: addr.postalCode || form.solicitorDetails?.postcode || "",
                });
              }}
      ...
      <div className="bg-white rounded-2xl border-2 border-slate-200 p-5">
          ...
          <div className="mb-5">
            ...
            <div className="grid sm:grid-cols-2 gap-3">
              {existingDocs.map((doc: any, i: number) => {
                ...
                return (
                  <div key={i} className="bg-white border-2 border-amber-200 rounded-xl overflow-hidden">
                    <div className="h-28 bg-slate-100 flex items-center justify-center relative">
                      {isImg ? (
                        <img src={fileUrl} alt={fileName} className="h-full w-full object-cover" onError={e => { (e.target as HTMLImageElement).style.display = "none"; }} />
                      ) : (
                        <div className="flex flex-col items-center gap-1">
                          <div className="size-10 bg-red-100 rounded-xl flex items-center justify-center">
                            <FileText className="size-5 text-red-600" />
                          </div>
                          <span className="text-xs font-bold text-slate-500">Document</span>
                        </div>
                      )}
                    ...
                    <div className="p-2.5">
                      <p className="text-xs font-bold text-slate-700 truncate mb-2">{fileName}</p>
                      <div className="flex gap-1.5">
                        ...
                        <button type="button" onClick={() => deleteExistingDoc(i)}
                          ...
                          {deletingIndex === i ? "..." : <Trash2 className="size-3.5" />}
                        </button>
                      </div>
                    </div>
                  </div>
        ...
        <div className="bg-slate-50 rounded-xl p-4 border-2 border-slate-200 mb-4">
          <p className="text-sm font-bold text-slate-700 mb-3">Add New Document</p>
          ...
          {selectedDocType && (selectedDocType !== "other" || customLabel) && (
            <div className="mt-3">
              <input type="file" id="editPrivateDocUpload" accept=".pdf,.jpg,.jpeg,.png,.doc,.docx" multiple className="hidden" onChange={handleFileSelect} />
              <label htmlFor="editPrivateDocUpload"
        ...
        {newDocs.length > 0 && (
          <div className="space-y-2">
            <p className="text-xs font-bold text-slate-600">New documents to upload ({newDocs.length}):</p>
            {newDocs.map((doc: any, i: number) => {
              const isImg = /\.(jpg|jpeg|png|gif|webp)$/i.test(doc.originalName || "");
              return (
                <div key={i} className="flex items-center gap-3 p-2.5 bg-white border-2 border-slate-200 rounded-xl">
                  {isImg && doc.preview ? (
                    <img src={doc.preview} className="size-9 rounded-lg object-cover flex-shrink-0" alt="" />
                  ) : (
                  ...
                  <button type="button" onClick={() => removeNewDoc(i)}
      ...
      <div className="bg-amber-50 border border-amber-200 rounded-xl p-3 flex items-start gap-2">
        ...
        <p className="text-xs text-amber-800 font-medium">
          Private documents are only accessible by King Property Auction staff and are never displayed on the public listing.
        </p>
      </div>
    </div>
  );
