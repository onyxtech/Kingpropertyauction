  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div><label className="block text-sm font-bold mb-1">Bid Deposit (£)</label><input type="number" min="0" onKeyDown={preventMinus} value={form.bidDepositAmount} onChange={(e) => { const v = e.target.value; if (v === '' || Number(v) >= 0) updateField("bidDepositAmount", v); }} className="w-full px-4 py-2.5 bg-white border-2 border-slate-200 rounded-xl text-sm" /></div>
        <div><label className="block text-sm font-bold mb-1">Max Bid Limit</label><input type="number" min="0" onKeyDown={preventMinus} value={form.maximumBidLimit} onChange={(e) => { const v = e.target.value; if (v === '' || Number(v) >= 0) updateField("maximumBidLimit", v); }} className="w-full px-4 py-2.5 bg-white border-2 border-slate-200 rounded-xl text-sm" /></div>
