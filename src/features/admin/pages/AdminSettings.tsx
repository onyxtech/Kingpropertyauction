...
import { Save, Send, CheckCircle, XCircle, Mail, Bell, FileText, Shield, Zap, AlertCircle } from "lucide-react";
...
export default function AdminSettings() {
  ...
  const [emailForm, setEmailForm] = useState({
    mailer: "smtp",
    host: "", port: "465", encryption: "SSL", username: "", password: "",
    sendgridApiKey: "",
    mailgunApiKey: "", mailgunDomain: "",
    mailchimpApiKey: "",
    senderName: "King Property Auction", senderEmail: "", replyTo: "",
  });
    ...
    const loadData = async () => {
      try {
        ...
        if (settingsResult.success && settingsResult.data) {
          setEmailForm(prev => ({ ...prev, ...settingsResult.data, password: "" }));
        }
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setSaveMessage(result.success ? "Settings saved successfully!" : "Error: " + result.message);
    } catch (e: any) {
  ...
  const handleRuleToggle = async (key: string) => {
    ...
    setRules(updated);
    try {
      await apiClient.fetch("/settings/rules", { method: "PUT", body: JSON.stringify(updated) });
    } catch (e) {}
  };

  const tabs = [
    { id: "email", label: "Email Configuration", icon: Mail, description: "SMTP settings, test emails, mailer preferences" },
    { id: "rules", label: "Notification Rules", icon: Bell, description: "Toggle automatic email notifications" },
    { id: "templates", label: "Email Templates", icon: FileText, description: "Customize email content and design" },
  ];
    ...
    <AdminLayout activeTab="settings" onTabChange={() => {}}>
      <div className="space-y-6">
        ...
        <div>
          ...
          <p className="text-slate-600 font-medium mt-1">Manage email configuration, notifications, and templates</p>
        </div>
            ...
            return (
              ...
              >
                <div className={`size-10 rounded-xl flex items-center justify-center mb-3 ${
                  isActive ? "bg-white/20" : "bg-gradient-to-br from-blue-500 to-indigo-600"
                }`}>
                  <Icon className={`size-5 ${isActive ? "text-white" : "text-white"}`} />
                </div>
                <h3 className={`font-black text-sm mb-1 ${isActive ? "text-white" : "text-slate-900"}`}>{tab.label}</h3>
                <p className={`text-xs font-medium ${isActive ? "text-white/70" : "text-slate-500"}`}>{tab.description}</p>
                {isActive && (
        ...
        <div className="bg-white/80 backdrop-blur-xl rounded-3xl border-2 border-white/60 shadow-xl overflow-hidden">
            ...
            <div className="flex items-center gap-3">
              {activeTabData && (
                <>
                  ...
                  <div>
                    <h3 className="text-lg font-black text-slate-900">{activeTabData.label}</h3>
                    <p className="text-xs text-slate-500 font-medium">{activeTabData.description}</p>
                  </div>
                </>
              )}
            </div>
          ...
          <div className="p-8">
              ...
              <div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                    {[
                      ...
                      <div key={field.key}>
                        ...
                        <input type="text" value={(emailForm as any)[field.key]}
                    ...
                    <div>
                      <label className="block text-xs font-bold text-slate-600 mb-1.5 uppercase tracking-wider">Encryption</label>
                      <select value={emailForm.encryption}
                    ...
                    {[
                      { label: "Username", key: "username", placeholder: "admin@kingpropertyauction.com" },
                      { label: "Password", key: "password", type: "password", placeholder: "Enter password" },
                    ].map(field => (
                      <div key={field.key}>
                        <label className="block text-xs font-bold text-slate-600 mb-1.5 uppercase tracking-wider">{field.label}</label>
                        <input type={field.type || "text"} value={(emailForm as any)[field.key]}
                          onChange={e => setEmailForm({ ...emailForm, [field.key]: e.target.value })}
                          className="w-full px-4 py-3 bg-white border-2 border-slate-200 rounded-xl text-sm font-medium focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all"
                          placeholder={field.placeholder} />
                      </div>
                ...
                {emailForm.mailer === "sendgrid" && (
                  <div className="p-5 bg-gradient-to-r from-blue-50 to-indigo-50 rounded-2xl border-2 border-blue-200">
                    ...
                    <div>
                      <label className="block text-xs font-bold text-slate-600 mb-1.5 uppercase tracking-wider">API Key</label>
                      <input type="password" value={emailForm.sendgridApiKey}
                      ...
                      {!emailForm.sendgridApiKey && (
                        <p className="flex items-center gap-1 text-xs text-amber-600 mt-2">
                          <AlertCircle className="size-3" /> API key required to send emails via SendGrid.
                        </p>
                      )}
                      <p className="text-xs text-slate-500 mt-1">Get your key at <a href="https://app.sendgrid.com/settings/api_keys" target="_blank" className="text-blue-600 underline">SendGrid Dashboard</a></p>
                    </div>
                  </div>
                )}
                ...
                {emailForm.mailer === "mailgun" && (
                  <div className="p-5 bg-gradient-to-r from-red-50 to-orange-50 rounded-2xl border-2 border-red-200">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div>
                        <label className="block text-xs font-bold text-slate-600 mb-1.5 uppercase tracking-wider">API Key</label>
                        <input type="password" value={emailForm.mailgunApiKey}
                      ...
                      <div>
                        <label className="block text-xs font-bold text-slate-600 mb-1.5 uppercase tracking-wider">Domain</label>
                        <input type="text" value={emailForm.mailgunDomain}
                    ...
                    {(!emailForm.mailgunApiKey || !emailForm.mailgunDomain) && (
                      <p className="flex items-center gap-1 text-xs text-amber-600 mt-2">
                        <AlertCircle className="size-3" /> API key and domain required to send emails via Mailgun.
                      </p>
                    )}
                    <p className="text-xs text-slate-500 mt-1">Get your key at <a href="https://app.mailgun.com/settings/api_security" target="_blank" className="text-red-600 underline">Mailgun Dashboard</a></p>
                  </div>
                )}
                ...
                {emailForm.mailer === "mailchimp" && (
                  <div className="p-5 bg-gradient-to-r from-amber-50 to-yellow-50 rounded-2xl border-2 border-amber-200">
                    ...
                    <div>
                      <label className="block text-xs font-bold text-slate-600 mb-1.5 uppercase tracking-wider">API Key</label>
                      <input type="password" value={emailForm.mailchimpApiKey}
                        onChange={e => setEmailForm({ ...emailForm, mailchimpApiKey: e.target.value })}
                      ...
                      {!emailForm.mailchimpApiKey && (
                        <p className="flex items-center gap-1 text-xs text-amber-600 mt-2">
                          <AlertCircle className="size-3" /> API key required to send emails via Mailchimp.
                        </p>
                      )}
                      <p className="text-xs text-slate-500 mt-1">Get your key at <a href="https://mandrillapp.com/settings" target="_blank" className="text-amber-600 underline">Mailchimp Transactional Settings</a></p>
                    </div>
                  </div>
                )}
                ...
                {!emailForm.mailer && (
                  <div className="p-5 bg-amber-50 rounded-2xl border-2 border-amber-200 flex items-center gap-3">
                    ...
                    <p className="text-sm text-amber-800 font-medium">Please select a mailer provider above to configure email settings.</p>
                  </div>
                )}
                ...
                <div className="mt-8 pt-6 border-t-2 border-slate-100">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
                    {[
                      { label: "Sender Name", key: "senderName", placeholder: "King Property Auction" },
                      { label: "Sender Email", key: "senderEmail", type: "email", placeholder: "noreply@kingpropertyauction.com" },
                      { label: "Reply-To Email", key: "replyTo", type: "email", placeholder: "info@kingpropertyauction.com" },
                    ].map(field => (
                      <div key={field.key}>
                        <label className="block text-xs font-bold text-slate-600 mb-1.5 uppercase tracking-wider">{field.label}</label>
                        <input type={field.type || "text"} value={(emailForm as any)[field.key]}
            ...
            {activeTab === "rules" && (
              <div>
                <p className="text-sm text-slate-600 mb-6">Enable or disable automatic email notifications for each event type.</p>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                  {Object.entries(ruleLabels).map(([key, label]) => (
                    <div key={key} className="flex items-center justify-between p-4 bg-slate-50 rounded-xl hover:bg-white hover:shadow-sm transition-all border-2 border-transparent hover:border-slate-200">
                      <span className="font-semibold text-slate-900 text-sm">{label}</span>
                      <button onClick={() => handleRuleToggle(key)}
                        className={`relative w-12 h-7 rounded-full transition-all duration-200 ${rules[key] !== false ? 'bg-green-500' : 'bg-slate-300'}`}>
                        <div className={`absolute top-0.5 size-6 bg-white rounded-full shadow-md transition-all duration-200 ${rules[key] !== false ? 'left-5' : 'left-0.5'}`} />
                      </button>
                    </div>
                  ))}
                </div>
            ...
            {activeTab === "templates" && (
              <div>
                <p className="text-sm text-slate-600 mb-6">Select a template to edit its subject and content. Full editor coming soon.</p>
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                  {Object.entries(ruleLabels).map(([key, label]) => (
                    <div key={key} className="p-5 bg-slate-50 rounded-2xl hover:bg-white hover:shadow-md transition-all border-2 border-transparent hover:border-blue-200 cursor-pointer group">
                      ...
                      <p className="text-xs text-slate-500 mt-1 font-mono">template: {key}</p>
                    </div>
                  ))}
                </div>
              </div>
        ...
        {saveMessage && (
          <div className="fixed bottom-6 right-6 z-50 animate-in slide-in-from-right duration-300">
            <div className={`px-5 py-4 rounded-2xl shadow-2xl backdrop-blur-xl border-2 flex items-center gap-3 ${
              saveMessage.includes('Error') ? "bg-red-500/95 border-red-400 text-white" : "bg-green-500/95 border-green-400 text-white"
            }`}>
              {saveMessage.includes('Error') ? <XCircle className="size-5 flex-shrink-0" /> : <CheckCircle className="size-5 flex-shrink-0" />}
              <span className="font-bold text-sm">{saveMessage}</span>
              <button onClick={() => setSaveMessage("")} className="ml-2 opacity-70 hover:opacity-100"><XCircle className="size-4" /></button>
            </div>
          </div>
        )}
        ...
        {testResult && (
          <div className="fixed bottom-6 right-6 z-50 animate-in slide-in-from-right duration-300">
            <div className={`px-5 py-4 rounded-2xl shadow-2xl backdrop-blur-xl border-2 flex items-center gap-3 ${
              testResult.success ? "bg-green-500/95 border-green-400 text-white" : "bg-red-500/95 border-red-400 text-white"
            }`}>
              {testResult.success ? <CheckCircle className="size-5 flex-shrink-0" /> : <XCircle className="size-5 flex-shrink-0" />}
              <div>
                <p className="font-bold text-sm">{testResult.success ? "Test Email Sent!" : "Test Failed"}</p>
                <p className="text-xs opacity-90">{testResult.success ? "Check your inbox to verify delivery." : testResult.message}</p>
              </div>
              <button onClick={() => setTestResult(null)} className="ml-2 opacity-70 hover:opacity-100"><XCircle className="size-4" /></button>
            </div>
          </div>
        )}
      </div>
